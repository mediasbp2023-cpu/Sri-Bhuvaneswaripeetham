...
export default function MahaPaadaYatraContributePage() {
  ...
  const upiNote = useMemo(() => {
    const base = `Maha Paada Yatra | ${donor.name || "Devotee"} | ${donor.mobile || ""}`.trim();
    return base;
  }, [donor]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-cream via-white to-brand-gold/10">
      <HeroBanner title="Donate to Sri Bhuvaneswari Peetham" description="Maha Paada Yatra" height="small" />
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex gap-3">
          ...
          <span className="px-4 py-2 rounded-lg bg-brand-maroon text-brand-gold font-semibold">Maha Paada Yatra</span>
          <a href="/contribute/arjita-seva" className="px-4 py-2 rounded-lg bg-brand-cream text-brand-maroon border border-brand-gold/30 hover:bg-brand-gold/10">Aarjita Seva / Pooja</a>
        </div>

        {step === 1 && (
          <div className="bg-white rounded-xl shadow-lg p-6 border border-brand-gold/20">
            <h2 className="text-2xl font-bold text-brand-maroon mb-4">Maha Paada Yatra Donation</h2>
